...
//se crean primero las columnas y luego se asigna la información
    ...
    private Object[][] obtenerMatrizDatos(ArrayList titulosList) throws SQLException {
        ...
        if(licenciasVigentes.isEmpty()){
            ...
            int fila = 0;
            ArrayList<String> licenciasVigentesAux = new ArrayList<String>();
            for (int i=0; i<licenciasVigentes.size(); i++) licenciasVigentesAux.add(licenciasVigentes.get(i));
...
// así como los tipos de datos que va a poder soportar.
    ...
    public void buscarTitular(String tipoDoc, String num_doc) throws SQLException {
        ...
        String nueva_info[][] = new String[informacion.length][titulosList.size()];

        int j=0;

        for(int i=0; i<filas; i++){
            System.out.println("info: "+informacion[i][2]);
            System.out.println("tipodoc: "+tipoDoc);
            System.out.println("info2: "+informacion[i][3]);
            System.out.println("nrodoc: "+num_doc);
            if(informacion[i][2].equals(tipoDoc) && informacion[i][3].equals(num_doc)){
                nueva_info[j] = (String[]) informacion[i];
                j++;
            }
        }

        if(nueva_info.length==0) {
